class Category:
  ...
  def deposit(self, amount, description=''):
      deposit_ = {'amount': amount, 'description': description}
      self.ledger.append(deposit_)

  def withdraw(self, amount, description=''):
      sufficient_funds = self.check_funds(amount)
      if sufficient_funds:
        withdrawal_ = {'amount': -amount, 'description': description}
  ...
  def __repr__(self):
    ...
    def create_spend_chart(categories):
        ...
        withdrawals = [
           -sum([exp['amount'] for exp in ledger_ if exp['amount'] < 0])
        ...
        cat_names = [
           cat.ljust(len(max(cat_names, key=len)), ' ') for cat in cat_names
        ]
        ...
        for idx, i in enumerate(percentages):
            ...
            if not i:
                ...
                for i in range(len(max(cat_names, key=len))):
                  chart_str.append(' ' * 5 + cats_str.format(*[cat[i] for cat in cat_names]))
